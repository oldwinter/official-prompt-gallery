...
NEXT = "next: node scripts/validate.mjs --mode authoring"
...
class ValidateAuthoringNextTests(unittest.TestCase):
    def test_publish_failure_names_authoring(self) -> None:
        ...
        self.assertIn("planned-publish", combined)
        self.assertIn("FAIL official image gallery validation (publish)", combined)
    ...
    def test_authoring_pass_has_no_publish_next(self) -> None:
        ...
        combined = f"{result.stdout}\n{result.stderr}"
        self.assertIn("PASS official image gallery validation (authoring)", combined)
        self.assertNotIn(NEXT, combined)

    def test_readme_says_current_checkout_needs_authoring(self) -> None:
        readme = README.read_text(encoding="utf-8")
        section = readme.split("## Validate", 1)[1].split("## Private capture flow", 1)[0]
